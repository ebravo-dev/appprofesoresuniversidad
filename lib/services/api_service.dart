...
class ApiService {
  ...
  /// Endpoint: POST /professors/register
  Future<Either<String, LoginResponse>> registerProfesor({
    ...
    try {
      ...
      Logger.info('Intentando registro para: $email');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Logger.info(
          'Registro exitoso para profesor: ${loginResponse.profesor.nombreCompleto}',
        );
        return Right(loginResponse);
      } else {
        final errorMessage = response.data['message'] ?? 'Error en el registro';
        Logger.error('Error en registro: $errorMessage');
        return Left(errorMessage);
      }
    } on DioException catch (e) {
      final errorMessage = _handleDioError(e);
      Logger.error('Error de conexión en registro: $errorMessage', e);
      return Left(errorMessage);
    } catch (e) {
